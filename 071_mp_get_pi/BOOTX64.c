...
void efi_main(void *ImageHandle __attribute__ ((unused)), struct EFI_SYSTEM_TABLE *SystemTable)
{
	...
	struct EFI_PROCESSOR_INFORMATION pi;
	status = msp->GetProcessorInfo(msp, 0, &pi);
	if (status) {
		puts(L"error: msp->GetProcessorInfo: status=0x", SystemTable);
		puts(int_to_unicode_hex(status, 16, str), SystemTable);
		puts(L"\r\n", SystemTable);
		while (1);
	}
	puts(L"ProcessorID: 0x", SystemTable);
	puts(int_to_unicode_hex(pi.ProcessorId, 16, str), SystemTable);
	puts(L"\r\n", SystemTable);
	puts(L"StatusFlag: 0x", SystemTable);
	puts(int_to_unicode_hex(pi.StatusFlag, 8, str), SystemTable);
	puts(L"\r\n", SystemTable);
	puts(L"Location:\r\n", SystemTable);
	puts(L"- Package: 0x", SystemTable);
	puts(int_to_unicode_hex(pi.Location.Package, 8, str), SystemTable);
	puts(L"\r\n", SystemTable);
	puts(L"- Core: 0x", SystemTable);
	puts(int_to_unicode_hex(pi.Location.Core, 8, str), SystemTable);
	puts(L"\r\n", SystemTable);
	puts(L"- Thread: 0x", SystemTable);
	puts(int_to_unicode_hex(pi.Location.Thread, 8, str), SystemTable);
	puts(L"\r\n", SystemTable);
