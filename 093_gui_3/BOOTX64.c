...
const struct EFI_GRAPHICS_OUTPUT_BLT_PIXEL COLOR_FG = {0xff, 0xff, 0xff, 0xff};
const struct EFI_GRAPHICS_OUTPUT_BLT_PIXEL COLOR_HIGHLIGHT = {0x00, 0xff, 0xff, 0xff};
void *IH;
	       ...
	       struct EFI_GRAPHICS_OUTPUT_BLT_PIXEL c)
{
	...
	for (i = x; i < (x + w); i++)
		draw_pixel(i, y, c);
	for (i = x; i < (x + w); i++)
		draw_pixel(i, y + h - 1, c);

	for (i = y; i < (y + h); i++)
		draw_pixel(x, i, c);
	for (i = y; i < (y + h); i++)
		draw_pixel(x + w - 1, i, c);
}
...
void draw_file_list(void)
{
	...
	for (file_entry = (struct FILE_LIST *)FILE_LIST_HEAD.next; FILE_LIST_HEAD.next != &FILE_LIST_HEAD;
		...
		draw_rect(file_entry->rect.x, file_entry->rect.y,
			  file_entry->rect.width, file_entry->rect.height, COLOR_FG);
...
void poll_mouse_pointer(void *_tmp __attribute__ ((unused)))
{
	...
	while (1) {
		...
		if (!status) {
			...
			if (!is_first) {
				draw_pixel(x, y, tmp[0]);
				draw_pixel(x + 1, y, tmp[1]);
				draw_pixel(x, y + 1, tmp[2]);
				draw_pixel(x + 1, y + 1, tmp[3]);
			} else {
				is_first = 0;
			}
...
{
	if ((r.x <= x) && (x <= (r.x + r.width - 1))
	    && (r.y <= y) && (y <= (r.y + r.height - 1)))
		return TRUE;
	return FALSE;
}
...
{
	unsigned long long status;
	unsigned int Event = 0xbeefcafe; /* dummy */
	struct FILE_LIST *file_entry;
	...
	status = MSP->StartupThisAP(MSP, poll_mouse_pointer, 1, (void *)&Event, 0, NULL, NULL);
	check_status(status, L"StartupThisAP");

	while (1) {
		for (file_entry = (struct FILE_LIST *)FILE_LIST_HEAD.next; FILE_LIST_HEAD.next != &FILE_LIST_HEAD;
		     file_entry = (struct FILE_LIST *)file_entry->list.next) {
			if (is_in_rect(pointer_x, pointer_y, file_entry->rect)) {
				if (!file_entry->is_highlight) {
					file_entry->is_highlight = 1;
					draw_rect(file_entry->rect.x, file_entry->rect.y,
						  file_entry->rect.width, file_entry->rect.height,
						  COLOR_HIGHLIGHT);
				}
			} else {
				if (file_entry->is_highlight) {
