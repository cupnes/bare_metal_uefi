...
void efi_main(void *ImageHandle __attribute__ ((unused)), struct EFI_SYSTEM_TABLE *SystemTable)
{
	...
	while (1) {
		...
		if (status) {
			puts(L"SystemTable->BootServices->CreateEvent: 0x", SystemTable);
		...
		if (status) {
			puts(L"SystemTable->BootServices->SetTimer: 0x", SystemTable);
			puts(int_to_unicode_hex(status, 16, str), SystemTable);
			puts(L"\r\n", SystemTable);
			while (1);
		}
		...
		puts(L"Wait.", SystemTable);
		status = SystemTable->BootServices->WaitForEvent(1, WaitList, &Index);
		if (status) {
			puts(L"SystemTable->BootServices->WaitForEvent: 0x", SystemTable);
			puts(int_to_unicode_hex(status, 16, str), SystemTable);
			puts(L"\r\n", SystemTable);
			while (1);
		}
