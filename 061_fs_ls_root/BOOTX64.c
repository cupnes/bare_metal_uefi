...
void efi_main(void *ImageHandle __attribute__ ((unused)), struct EFI_SYSTEM_TABLE *SystemTable)
{
	...
	puts(L"sizeof EFI_FILE_INFO: ", SystemTable);
	puts(int_to_unicode(sizeof(struct EFI_FILE_INFO), 5, str), SystemTable);
	puts(L"\r\n", SystemTable);

	struct EFI_FILE_INFO *efi;
	while (1) {
		status = root->Read(root, &buf_size, (void *)file_buf);
		puts(L"status: root->Read: ", SystemTable);
		...
		puts(L"FileName: ", SystemTable);
		puts(efi->FileName, SystemTable);
		puts(L"\r\n", SystemTable);
