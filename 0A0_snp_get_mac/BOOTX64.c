...
struct EFI_NETWORK_STATISTICS {
	unsigned long long RxTotalFrames;
	unsigned long long RxGoodFrames;
	unsigned long long RxUndersizeFrames;
	unsigned long long RxOversizeFrames;
	unsigned long long RxDroppedFrames;
	unsigned long long RxUnicastFrames;
	unsigned long long RxBroadcastFrames;
	unsigned long long RxMulticastFrames;
	unsigned long long RxCrcErrorFrames;
	unsigned long long RxTotalBytes;
	unsigned long long TxTotalFrames;
	unsigned long long TxGoodFrames;
	unsigned long long TxUndersizeFrames;
	unsigned long long TxOversizeFrames;
	unsigned long long TxDroppedFrames;
	unsigned long long TxUnicastFrames;
	unsigned long long TxBroadcastFrames;
	unsigned long long TxMulticastFrames;
	unsigned long long TxCrcErrorFrames;
	unsigned long long TxTotalBytes;
	unsigned long long Collisions;
	unsigned long long UnsupportedProtocol;
};
...
struct EFI_SIMPLE_NETWORK_MODE {
	...
	unsigned char MacAddressChangeable;
	unsigned char MultipleTxSupported;
	unsigned char MediaPresentSupported;
...
struct EFI_SIMPLE_NETWORK_PROTOCOL {
	...
	unsigned long long (*Initialize)(
		struct EFI_SIMPLE_NETWORK_PROTOCOL *This,
		unsigned long long ExtraRxBufferSize,
		unsigned long long ExtraTxBufferSize);
	unsigned long long (*Reset)(
		struct EFI_SIMPLE_NETWORK_PROTOCOL *This,
		unsigned char ExtendedVerification);
	unsigned long long (*Shutdown)(
		struct EFI_SIMPLE_NETWORK_PROTOCOL *This);
	unsigned long long (*ReceiveFilters)(
		struct EFI_SIMPLE_NETWORK_PROTOCOL *This,
		unsigned int Enable,
		unsigned int Disable,
		unsigned char ResetMCastFilter,
		unsigned long long MCastFilterCnt,
		struct EFI_MAC_ADDRESS *MCastFilter);
	unsigned long long (*StationAddress)(
		struct EFI_SIMPLE_NETWORK_PROTOCOL *This,
		unsigned char Reset,
		struct EFI_MAC_ADDRESS *New);
	unsigned long long (*Statistics)(
		struct EFI_SIMPLE_NETWORK_PROTOCOL *This,
		unsigned char Reset,
		unsigned long long *StatisticsSize,
		struct EFI_NETWORK_STATISTICS *StatisticsTable);
	unsigned long long (*MCastIpToMac)(
		struct EFI_SIMPLE_NETWORK_PROTOCOL *This,
		unsigned char IPv6,
		union EFI_IP_ADDRESS *IP,
		struct EFI_MAC_ADDRESS *MAC);
	unsigned long long (*NvData)(
		struct EFI_SIMPLE_NETWORK_PROTOCOL *This,
...
#endif /* QEMU */
struct EFI_SIMPLE_NETWORK_PROTOCOL *SNP;
struct LIST FILE_LIST_HEAD;
unsigned short BUF_STR[1024];
unsigned char SPS_LEFT_BUTTON;
...
void check_status(unsigned long long status, unsigned short *name)
{
	...
	if (status) {
		...
		puts(name);
		puts(L": ");
		puts(int_to_unicode_hex(status, 16, str));
		puts(L"\r\n");
		while (1);
	}
}
...
#endif /* QEMU */
	struct EFI_GUID snp_guid = {0xa19832b9, 0xac25, 0x11d3, {0x9a, 0x2d, 0x00, 0x90, 0x27, 0x3f, 0xc1, 0x4d}};

	IH = ImageHandle;
	ST = SystemTable;

	status = SystemTable->BootServices->LocateProtocol(&gop_guid, NULL, (void **)&GOP);
	check_status(status, L"LocateProtocol(GOP)");

	status = SystemTable->BootServices->LocateProtocol(&sfsp_guid, NULL, (void **)&SFSP);
	check_status(status, L"LocateProtocol(SFSP)");

	status = SystemTable->BootServices->LocateProtocol(&spp_guid, NULL, (void **)&SPP);
	check_status(status, L"LocateProtocol(SPP)");
	SPS_LEFT_BUTTON = 0;

#ifndef QEMU
	status = SystemTable->BootServices->LocateProtocol(&msp_guid, NULL, (void **)&MSP);
	check_status(status, L"LocateProtocol(MSP)");
#endif /* QEMU */

	status = SystemTable->BootServices->LocateProtocol(&snp_guid, NULL, (void **)&SNP);
	check_status(status, L"LocateProtocol(SNP)");
}
...
void draw_file_list(void)
{
	...
	check_status(status, L"OpenVolume");
	...
	while (buf_size) {
		...
		if (buf_size) {
			...
			status = ST->BootServices->AllocatePool(
				...
				(void **)&file_entry);
			check_status(status, L"AllocatePool");
	...
	check_status(status, L"Close");
}

void poll_mouse_pointer(void *_tmp __attribute__ ((unused)))
{
	unsigned long long status;
	...
	check_status(status, L"Reset(SPP)");

	while (1) {
		status = SPP->GetState(SPP, &sps);
		if (!status) {
			SPS_LEFT_BUTTON = sps.LeftButton;
			if (!is_first) {
...
void efi_main(void *ImageHandle, struct EFI_SYSTEM_TABLE *SystemTable)
{
	...
	for (i = 0; i < 32; i++) {
		puts(int_to_unicode_hex(SNP->Mode->CurrentAddress.Addr[i], 2, BUF_STR));
		puts(L" ");
	}
	puts(L"\r\n");
